...
def get_cv_segments(text: str) -> Dict[str, str]:
    ...
    SECT_MAP = {
        ...
        "experience_junk": ["chipsnbytes", "technolog", "developer", "engineer", "designer"] # For dirty data detection
    }
             ...
             for key, synonyms in SECT_MAP.items():
                 ...
                 if any(l_clean == s or l_clean.startswith(s + " ") or l_clean.startswith(s + ":") or (s in l_clean and len(l_clean) < len(s) + 5) for s in synonyms):
                     ...
                     if key == "experience_junk" and current_key == "skills":
                         logger.info(f"Dirty Skill Data Detected - Switching to Meta at: '{l_orig}'")
                         current_key = "meta"
                         found_new = True
                         break
...
def extract_skills_it(text: str, candidate_name: str = "") -> Dict[str, List[str]]:
    ...
    junk_skills = ["communication", "team work", "interpersonal", "fast learning", "highly adaptive", "flexible", "team player", "problem solving"]

    for line in text.split('\n'):
        line = line.strip()
        if not line or len(line) > 150: continue
        if any(j in line.lower() for j in junk_skills): continue
        ...
        for part in parts:
            ...
            is_tech = any(re.search(r'\b' + re.escape(c) + r'\b', t_low) for c in core_tech_dict)
            is_tool = any(re.search(r'\b' + re.escape(tool) + r'\b', t_low) for tool in industry_tools_dict)
            
            if is_tech:
                if t not in tech_skills: tech_skills.append(t)
            elif is_tool:
                if t not in tech_tools: tech_tools.append(t)
            else:
                # If it's a short, capitalized word or acronym, keep it as skill as fallback
                if len(t) <= 12 and any(c.isupper() for c in t):
                    if t not in tech_skills: tech_skills.append(t)
...
async def process_cv_to_template(file: UploadFile = File(...), template_name: str = "default"):
    ...
    try:
        ...
        skills_obj = extract_skills_it(segs["skills"], personal["name"])
...
async def process_cv(file: UploadFile = File(...)):
    ...
    try:
        ...
        segs = get_cv_segments(text)
        skills_obj = extract_skills_it(segs.get("skills", ""), fullname)
