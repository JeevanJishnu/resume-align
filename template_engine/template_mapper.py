...
def _replicate_dynamic_sections(doc, data):
    ...
    # 2. Assign sections to elements, distinguishing between top-level section headers and record-level headers
    element_sections = []
    ...
    for elem in top_elements:
        ...
        if is_top_level_section_header:
            ...
            # If it's not a top-level section header, it belongs to the current section's content/block
    ...
    for stype, records in SECTION_RECORDS.items():
        ...
        for e in section_elements:
            elem = e['elem']
            stype_check, score, is_record_check = is_section_header(elem)
            txt = elem.text.lower() if isinstance(elem, Paragraph) else ""
            
            # A block starts if it's a section/sub-section header OR the very first item
            is_block_start = (stype_check != SectionType.UNKNOWN and score >= 0.7)
            # is_record_check is already true for things like "Project #1"
            if is_record_check: is_block_start = True
            
            if is_block_start or not curr_block:
        ...
        # Determine if the first block is just a 'Section Title' (e.g. "Project Profile")
        if len(blocks) > 1:
            first_elem = blocks[0][0]
            if isinstance(first_elem, Paragraph):
                txt = first_elem.text.lower()
                # If it doesn't look like a numbered record (#1), it's likely a section title
                if "#" not in txt and any(k in txt for k in ["profile", "details", "experience", "history"]):
                    blocks.pop(0) # Remove section title from replication candidates

        if not blocks: continue
        
        num_existing = len(blocks)
        num_needed = len(records)
        
        if num_needed > num_existing:
            blueprint = blocks[-1]
            last_elem = blueprint[-1]
            
            for i in range(num_existing + 1, num_needed + 1):
                for b_elem in blueprint:
                    new_xml = deepcopy(b_elem._element)
                    _sanitize_xml(new_xml)
                    last_elem._element.addnext(new_xml)
                    
                    if isinstance(b_elem, Paragraph):
                        new_obj = Paragraph(new_xml, doc)
                        # Increment number if present (e.g., Project #1 -> Project #2)
                        if "#" in new_obj.text:
                            new_obj.text = re.sub(r'#\d+', f'#{i}', new_obj.text)
                    else:
                        new_obj = Table(new_xml, doc)
                    
                    last_elem = new_obj
            logger.info(f"Replicated {stype}: Scaled up from {num_existing} to {num_needed} blocks.")
        elif num_needed < num_existing:
            # Scale down: Remove blocks from the end (works even if num_needed is 0 or 1)
            # Scale down: Remove blocks from the end
            for i in range(num_existing - 1, num_needed - 1, -1):
                block_to_remove = blocks[i]
                for b_elem in block_to_remove:
                    try:
                        b_elem._element.getparent().remove(b_elem._element)
                    except:
                        pass
            logger.info(f"Replicated {stype}: Scaled down from {num_existing} to {num_needed} blocks.")
